...
const Auth = ({ setOpen }: Props) => {
  ...
  const handleMagicLink = async (emailValue: string) => {
    try {
      setError("");
      setSuccess(false);
      if (!emailValue.trim()) { setError("Email is required"); return; }
      if (!validateEmail(emailValue)) { setError("Please enter a valid email address"); return; }
      setIsLoading(true);
      const res = await magicLinkAPI(emailValue);
      ...
      setError((err as { response?: { data?: { msg?: string } } })?.response?.data?.msg || "Failed to send magic link. Please try again.");
    } finally {
      setIsLoading(false);
    }
  };
