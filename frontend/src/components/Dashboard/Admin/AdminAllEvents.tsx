import { Calendar, Search, MapPin, IndianRupee, Users, Edit2, Check } from "lucide-react";
import { useEffect, useState } from "react";
import { getAllEvent, updateEventApi } from "@/api/event/eventApi";
import type { eventI } from "@/types/Event";
import { EventEditModal } from "./AdminEditEventModal";

export function AdminAllEvents() {
  ...
  const handleSaveEvent = async (slug: string, data: any) => {
    try {
      setError("");
      setSuccess("");
      const res = await updateEventApi(slug, data);
      if (res?.success) {
        setSuccess("Event updated successfully!");
        setEvents(events.map(ev => ev.slug === slug ? { ...ev, ...data } : ev));
        setTimeout(() => setSuccess(""), 4000);
        return true;
      } else {
        setError(res?.msg || "Failed to update event");
        return false;
      }
    } catch (err) {
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-[#4b6480] text-sm mt-1">
            Browse and manage all events created across the platform.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-5">
        {loading ? (
            ...
            <div>
              ...
              <p className="text-[#4b6480] text-sm mt-1">
                {searchQuery
                  ...
                  : "There are no events currently available on the platform."}
              </p>
            </div>
          </div>
        ) : (
          filteredEvents.map((event) => (
            <div
              key={event.id}
              className="group bg-[#0d1117] border border-[#1e2d3d] hover:border-[#243447] rounded-3xl overflow-hidden transition-all duration-300 flex flex-col h-full hover:shadow-2xl hover:shadow-black/50"
            >
