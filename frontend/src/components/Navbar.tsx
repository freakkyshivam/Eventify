...
import { fetchUser, handleLogout } from "@/api/userApi";
...
const Navbar = () => {
  ...
  const handleMagicLink = async (email: string) => {
    try {
      ...
      if (res.success) {
        setSuccess(true);
        setEmail("");
        setTimeout(() => { setShowAuthModal(false); setSuccess(false); }, 3000);
      } else {
  ...
  const roleInitial = userRole === "admin" ? "A" : userRole === "organizer" ? "O" : "U";
  const roleLabel = userRole === "admin" ? "Admin" : userRole === "organizer" ? "Organizer" : "User";
            ...
            {isAuthenticated ? (
              <>
                ...
                <div className="flex items-center gap-2.5 pl-2 border-l border-white/10">
                  ...
                  <div className="w-9 h-9 rounded-xl bg-linear-to-br from-violet-600 to-fuchsia-600 flex items-center justify-center cursor-pointer hover:scale-105 hover:shadow-[0_0_15px_rgba(124,58,237,0.5)] transition-all duration-200 border border-violet-500/30">
                    <span className="text-white font-bold text-xs">{roleInitial}</span>
                  </div>
                </div>
              </>
            ) : (
